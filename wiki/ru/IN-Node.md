...
Внутри выполняется преобразование выдаваемого формата под HomeKit и ноду можно стыковать прямо с homekit-нодой, в результате чего значительно упрощается flow. Юзкейсы можно найти в разделе [Примеры](Examples).

Для homekit formatted выдачи имеются опции:
- **Unique messages** — отправляются только уникальные сообщения, без дубликатов.
- **Homekit format** — выбор вывода под разные устройства — Smart Speaker и Television. В [примерах использования](Examples) описаны сценарии.
